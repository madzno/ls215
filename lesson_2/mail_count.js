...
function formatDate(date) {
  ...
  return [weekday, month, dayOfMonth, year].join(' ');
}
...
function mailCount(emailData) {
  ...
  let emailParts = emails.map(emailString => emailString.split('#/#'));
  let emailDates = emailParts.map(emailArray => new Date(emailArray[2].replace(/[^0-9\-]/g, '')));

  emailDates.sort((date1, date2) => {
    if (date1 < date2) {
      return -1;
    } else if (date1 > date2) {
      return 1;
  ...
  console.log(`Date Range: ${formatDate(emailDates[0])} - \
${formatDate(emailDates[emailDates.length - 1])}`);
